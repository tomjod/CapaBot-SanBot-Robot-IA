...
- `src/` — Android app (Java) running on the Sanbot robot (Android 21+, Sanbot OpenSDK). Built via Gradle. Currently mid-migration to Clean Architecture + MVVM (see commit `924a7fe`).
- `backend/` — Python FastAPI service ("Visitor Notify Assistant MVP"). Brokers notifications across Telegram, Email and WhatsApp; hosts a Telegram onboarding bot; serves an admin HTTP UI.
- `whatsapp-bridge/` — Node/TypeScript microservice wrapping Baileys (`@whiskeysockets/baileys`). Exposes a small REST API the backend calls to send WhatsApp messages.
...
pytest backend/tests/unit/test_notification_service.py
pytest backend/tests/unit/test_notification_service.py::test_submit_routes_whatsapp
```

There is no `pyproject.toml` / `setup.cfg`; tests rely on the package being importable from the repo root (`backend.app.*`). Run pytest from the repo root.
...
Target SDK: `compileSdkVersion 26`, `minSdkVersion 21`. The Sanbot SDK is vendored under `libs/SanbotOpenSDK_2.0.1.10.aar`.

## Backend Architecture

The backend follows Clean / Hexagonal Architecture. Layers are physical directories under `backend/app/`:
...
- `bootstrap/` — composition root.
  - `settings.py` (`BackendSettings.from_env`) resolves every env var (prefix: `VISITOR_NOTIFY_*`) and runtime data paths. It checks `backend/runtime_data/<file>.json` first, then legacy `backend/app/data/<file>.json`.
...
The backend's `WhatsAppBridgeProvider` (`backend/app/infra/providers/whatsapp_provider.py`) is the only intended client.
