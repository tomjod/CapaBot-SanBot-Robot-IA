...
class SupportsMailtrapClient(Protocol):
    def send(self, mail: Any) -> Mapping[str, object]:
        ...
...
@dataclass(frozen=True)
class _FallbackMail:
    sender: _FallbackAddress
    to: list[_FallbackAddress]
    subject: str
    text: str


def _mailtrap_types() -> tuple[type[Any], type[Any]]:
    try:
        from mailtrap import Address, Mail

        return Address, Mail
    except ModuleNotFoundError:
        return _FallbackAddress, _FallbackMail
